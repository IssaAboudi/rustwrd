...
fn editorMoveCursor(terminal: &mut Terminal, key: u8) -> io::Result<()> {
    match key {
        ARROW_LEFT!() => terminal.curs_x -= 1,
        ARROW_RIGHT!() => terminal.curs_x += 1,
        ARROW_UP!() => terminal.curs_y -= 1,
        ARROW_DOWN!() => terminal.curs_y += 1,
